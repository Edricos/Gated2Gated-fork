...
class Trainer:
    ...
    def __init__(self, options: TrainingOptions) -> None:
        ...
        if self.opt.model_type == "multioutput":
            self.models["gated2gated"] = networks.PackNetSlim01MultiDecoder(
            ...
            if self.opt.depth_model == "packnet":
                self.models["depth"] = networks.PackNetSlim01(
                    dropout=self.opt.dropout,
                    version="{}{}".format(1, self.opt.feat_stack)
                )
            ...
            self.depth_normalizer = torch.tensor(
                self.opt.depth_normalizer,
                requires_grad=self.opt.train_depth_normalizer,
                device=self.device
            )
        ...
        if self.opt.intensity_mask:
            ...
            crop_size_h, crop_size_w = (int((full_res_shape[1] - self.opt.height) / 2),
                                        int((full_res_shape[0] - self.opt.width) / 2))
            assert os.path.isfile(os.path.join(
                self.opt.depth_flat_world_fpath,
                'depth_flat_world.npz')), "path to flat depthmap is required for using intensity mask!"
            self.depth_flat_world = np.load(os.path.join(
                self.opt.depth_flat_world_fpath,
                'depth_flat_world.npz')
            )['arr_0']
            self.depth_flat_world = self.depth_flat_world[:, :,
                                    crop_size_h:(self.depth_flat_world.shape[2] - crop_size_h),
                                    crop_size_w:(self.depth_flat_world.shape[3] - crop_size_w)
                                    ]
        ...
        self.depth_metric_names = ["de/abs", "de/abs_rel", "de/sq_rel", "de/rms", "de/log_rms",
                                   "da/a1", "da/a2", "da/a3"]
    ...
    def generate_images_pred(self, inputs, outputs):
        ...
        for scale in self.opt.scales:
            ...
            ################################################################################################################################
            '''
                根据深度的强度剖面计算重建以重新生成中心帧
            '''
            if self.opt.cycle_loss and scale == 0:  # 仅对原始分辨率进行门控重建，即比例 = 0
    ...
    def compute_losses(self, inputs, outputs):
        ...
        for scale in self.opt.scales:  # 0
            loss = 0
            reprojection_losses = []
            ...
            disp = outputs[("disp", scale)]
            depth = outputs[("depth", 0, scale)].detach()  # 使用分离来避免传递梯度
            gated = inputs[("gated", 0, scale)]
            target = inputs[("gated", 0, source_scale)]

            if self.opt.temporal_loss:  # True
                ...
                if self.opt.infty_hole_mask and self.epoch >= self.opt.infty_epoch_start:
                    ...
                    # 使用门控强度识别靠近相机的像素  0.995
                    close_pixel_mask = ((target[:, 0, :, :] >= self.opt.close_px_fact * target[:, 2, :, :]) |
                                        (target[:, 1, :, :] >= self.opt.close_px_fact * target[:, 2, :, :]))

                    # 为位于 2*中值和最大深度之间的像素构建掩模   4
                    ...
                    depth_lower_thresh = (depth_lower_thresh.reshape(self.opt.batch_size, 1, 1, 1) *
                                          torch.ones_like(depth))
                    depth_higher_thresh = (depth_higher_thresh.reshape(self.opt.batch_size, 1, 1, 1) *
                                           torch.ones_like(depth))
                    far_depth_mask = (depth >= depth_lower_thresh) & (depth <= depth_higher_thresh)

                    # 这里的想法是，被识别为更接近门控强度的像素应该具有较低的深度
                    # 也是如此。任何被识别为接近门控强度但深度大于 2*中值的像素
                    # 深度可能对应于无限孔，应将其取出用于重投影损失计算。
                    infty_holes_mask = far_depth_mask * close_pixel_mask.unsqueeze(1)

                    # 不是无穷大掩模，因为我们想要保留不是无穷大掩模的像素
                    # 无限像素（很可能是无限像素）
        ...
        for j in range(min(4, self.opt.batch_size)):  # write a maxmimum of four images
            for s in self.opt.scales:
                ...
                if self.opt.temporal_loss:
                    ...
                    if self.opt.infty_hole_mask and self.epoch >= self.opt.infty_epoch_start:
                        writer.add_image("close_pix_mask_{}/{}".format(s, j),
                                         outputs["close_pix_mask/{}".format(s)][j][None, ...], self.step)
                        writer.add_image("infty_hole_mask_{}/{}".format(s, j),
                                         outputs["inf_holes_mask/{}".format(s)][j], self.step)
                ...
                if self.opt.cycle_loss and s == 0:
                    ...
                    writer.add_image("scaled_depth_pred_{}_{}/{}".format(frame_id, s, j),
                                     depth_image(outputs[("sc_depth", s)][j].data[0].detach().cpu()), self.step,
                                     dataformats='HWC')
                ...
                writer.add_image("depth_pred_{}_{}/{}".format(frame_id, s, j), depth_img[:, :, 0:3], self.step,
                                 dataformats='HWC')
